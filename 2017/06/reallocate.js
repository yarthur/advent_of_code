var reallocate = function (memoryBanks) {
    var memoryBankCount = memoryBanks.length,
        snapshots = new Set(),
        reallocationCount = 0;
    while (snapshots.has(memoryBanks.join(",")) !== true) {
        // snapshots will be one more than snapshots, due to the original configuration (pre-reallocation).
        snapshots.add(memoryBanks.join(","));

        let { heavyVal, allocationIndex } = memoryBanks.reduce(
            function (retObj, bankVal, index) {
